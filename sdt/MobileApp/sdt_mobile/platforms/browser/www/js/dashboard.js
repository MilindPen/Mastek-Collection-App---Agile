...
var dashboard = {
    ...
    getDashboardDataFromOfflineDB: function(){
    	...
    	var weekStartDate = dataStorage.getData(WEEK_START_DATE);
    	var weekEndDate = dataStorage.getData(WEEK_END_DATE);	
    	var dashboardDataParams = [journeyId,weekStartDate,weekEndDate,journeyId,userId,weekStartDate,weekEndDate,journeyId,userId,weekStartDate,weekEndDate,journeyId,userId,weekStartDate,weekEndDate,journeyId,userId,weekStartDate,weekEndDate,weekStartDate,weekEndDate];
    	...
    	dbObj.transaction(function(tx) {	  
			tx.executeSql(dbQueries.selectDashboardData,dashboardDataParams,function(tx, res) {
                ...
                if(len > 0) {
				    ...
				    dailyBalanceArray = dailyBalanceArray.filter(function(n){ return n != undefined }); 
				    ...
				    console.log(JSON.parse(dailyBalanceArray[0]));
					...
					for(var i=0;i<dailyBalanceArray.length;i++){
						...
						for(var j=0;j<jsonObj1.length;j++){
							if(JSON.parse(dailyBalanceArray[i])==jsonObj1[j].Date){
						...
						hashMap1.set(JSON.parse(dailyBalanceArray[i]).toString(),hashMap2);
