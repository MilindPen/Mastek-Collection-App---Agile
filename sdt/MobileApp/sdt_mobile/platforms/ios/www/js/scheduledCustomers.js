...
var SCList = {		
        ...
        $('.searchbox-input').on('keyup',function(e){
            ...
            if(len > 2){
                ...
                for(var i = 0; i < tagElems.length; i++){
                    ...
                    var searchElementName = $(liChild2).children('span.Name');      
                    var searchElementAddress = $(liChild1).children('div.address');
                    var searchElements=(searchElementName.text().toLowerCase())+(searchElementAddress.text().toLowerCase());
                    if((searchElements).search($(this).val().toLowerCase()) > -1){
                        $(tag).show();
                    }
                }
            }
    ...
    searchHighlightDaily: function(){    
        $('#search-input').bind('keyup change', function(ev) {
	        ...
	        var searchElementName = $(liChild2).children('span.Name');
	        var searchElementAddress = $(liChild1).children('div.address');
	        $(liChild2).children('span.Name').removeHighlight();
	        $(liChild1).children('div.address').removeHighlight();
	        
	        // enable highlight when user enters minimum of 3 characters
	        if ( searchTerm.length > 2 ) {
	            // highlight the searched term
	        	$(liChild2).children('span.Name').highlight( searchTerm );
	            $(liChild1).children('div.address').highlight( searchTerm );
	        }
        });
    },
    ...
    getDailyCustListFromOfflineDB: function(dbObj){
	    ...
	    dbObj.transaction(function(tx) {	  
			tx.executeSql(selectSCDailyListStatement,selectedVisitDateParam,function(tx, res) {
    ...
    getWeeklyCustListFromOfflineDB: function(dbObj){
        ...
        //execute select query
	    dbObj.transaction(function(tx) {	  
			tx.executeSql(selectSCWeeklyListStatement,[],function(tx, res) {
    ...
    renderSCDailyList: function(len,res){
        ...
        totalTermAmount = (!listItem.TotalTermAmount) ? "0":parseInt(listItem.TotalTermAmount);
		totalPaidAmount = (!listItem.TotalPaidAmount) ? "0":parseInt(listItem.TotalPaidAmount);
		...
		$('ul.dailyList').append('<li class="cust ui-li-static ui-body-inherit" id=list'+idCounter+' onclick=SCList.callCustDetails('+listItem.CustomerNumber+')>');
                 ...
                 switch(status){
                    ...
                    case statusType.CNSO:    
                        $('li#list'+idCounter).append('<div class=vwhite>&nbsp;</div><div id="" class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                        SCList.commonDailyListElements(idCounter,fullName,address);
                        break;
                    case statusType.CPA:
                        $('li#list'+idCounter).append('<div class=vgreen>&nbsp;</div><div id="" class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                        SCList.commonDailyListElements(idCounter,fullName,address);
                        $('li#list'+idCounter).append('<div class="Amount"><span class="paidAmt">&pound;'+totalPaidAmount+'&nbsp;</span>(<span class="weeklyTermAmt">&pound;'+totalTermAmount+'</span>)</div></li>');
                        break;
                    case statusType.COMP:
                        ...
                        break;
                    case statusType.CPD:
                        $('li#list'+idCounter).append('<div class=vamber>&nbsp;</div><div id="" class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                        SCList.commonDailyListElements(idCounter,fullName,address);
						$('li#list'+idCounter).append('<div class="Amount"><span class="paidAmt">&pound;'+totalPaidAmount+'&nbsp;</span>(<span class="weeklyTermAmt">&pound;'+totalTermAmount+'</span>)</div></li>');
                        break;
                    ...
                    case statusType.SWP:
                    case statusType.SCP:    
                        $('li#list'+idCounter).append('<div class=vred>&nbsp;</div><div id="" class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                        SCList.commonDailyListElements(idCounter,fullName,address);
                        $('li#list'+idCounter).append('<div class="Amount"><span class="paidAmt">&pound;'+totalPaidAmount+'&nbsp;</span>(<span class="weeklyTermAmt">&pound;'+totalTermAmount+'</span>)</div></li>');
                        break;
                    case (statusType.NW):
                        $('li#list'+idCounter).append('<div class=vblue>&nbsp;</div><div id="" class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                    	SCList.commonDailyListElements(idCounter,fullName,address);
                        break;
                    default:
                        break;
            	...
            	if(!$(lvl3).has("li").length){
                    ...
                    if(len > 2){
	            		...
	            		for(j=0;j<liLen;j++){
	                        ...
	                        var searchElementName = $(lvl6).children('span.Name');      
	                        var searchElementAddress = $(lvl5).children('div.address');
	                        var searchElements=(searchElementName.text().toLowerCase())+(searchElementAddress.text().toLowerCase());
                    	...
                    	for(j=0;j<liLen;j++){
	            			...
	            			//$(lvl6).children('span.Name').removeHighlight();
	            	        //$(lvl5).children('div.address').removeHighlight();
                    	}
            	...
            	if(!$(lvl3).has("li").length){
            		...
            		console.log("i: "+i+"liLen "+$(lvl3).children('li').length);
	            		...
	            		for(j=0;j<liLen;j++){
	            	        ...
	            	        //enable search if user enters minimum of 3 characters
	                        if(len > 2){
	                        	$(lvl6).children('span.Name').highlight(searchTerm);
	            	        	$(lvl5).children('div.address').highlight(searchTerm);
    ...
    renderSCWeeklyList: function(lenWeekly,res){
		...
		for(i=currBucket;i<datesBucket.length;i++){
			...
			if(datesBucket[i]==listItem.VisitDate){
				...
				switch(status){
                    case statusType.PEND:
                    case statusType.CNSO:    
                        $('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class=vwhite>&nbsp;</div><div id=weeklyDay'+currBucket+'jOrder'+bucketListCounter+' class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                        SCList.commonWeeklyListElements(currBucket,bucketListCounter,fullName,address);
                        break;
                    case statusType.CPA:
                    	$('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class=vgreen>&nbsp;</div><div id=weeklyDay'+currBucket+'jOrder'+bucketListCounter+' class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                        SCList.commonWeeklyListElements(currBucket,bucketListCounter,fullName,address);
                        $('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class="Amount"><span class="paidAmt">&pound;'+totalPaidAmount+'&nbsp;</span>(<span class="weeklyTermAmt">&pound;'+totalTermAmount+'</span>)</div></li>');
                        break;
                    case statusType.COMP:
                    	$('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class=vgreen>&nbsp;</div><div id=weeklyDay'+currBucket+'jOrder'+bucketListCounter+' class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                        SCList.commonWeeklyListElements(currBucket,bucketListCounter,fullName,address);
                        ...
                        break;
                    case statusType.CPD:
                    	$('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class=vamber>&nbsp;</div><div id=weeklyDay'+currBucket+'jOrder'+bucketListCounter+' class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                        SCList.commonWeeklyListElements(currBucket,bucketListCounter,fullName,address);
                        $('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class="Amount"><span class="paidAmt">&pound;'+totalPaidAmount+'&nbsp;</span>(<span class="weeklyTermAmt">&pound;'+totalTermAmount+'</span>)</div></li>');
                        break;
                    case statusType.CNSC:
                    case statusType.SWP:
                    case statusType.SCP:    
                    	$('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class=vred>&nbsp;</div><div id=weeklyDay'+currBucket+'jOrder'+bucketListCounter+' class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                        SCList.commonWeeklyListElements(currBucket,bucketListCounter,fullName,address);
                        $('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class="Amount"><span class="paidAmt">&pound;'+totalPaidAmount+'&nbsp;</span>(<span class="weeklyTermAmt">&pound;'+totalTermAmount+'</span>)</div></li>');
                        break;
                    case (statusType.NW):
                    	$('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class=vblue>&nbsp;</div><div id=weeklyDay'+currBucket+'jOrder'+bucketListCounter+' class=journeyOrder>'+listItem.JourneyOrderBy+'</div>');
                    	SCList.commonWeeklyListElements(currBucket,bucketListCounter,fullName,address);
                        break;
                    default:
                        break;
	 ...
	 commonWeeklyListElements: function(currBucket,bucketListCounter,fullName,address){
         ...
         $('div#weeklyDay'+currBucket+'nc'+bucketListCounter).append('<span class=Name>'+fullName+'</span></div>');
         $('div#weeklyDay'+currBucket+'nca'+bucketListCounter).append('<div class=address>'+address+'</div></div></li>');
     ...
     commonDailyListElements: function(idCounter,fullName,address){
            ...
            $('div#nc'+idCounter).append('<span class=Name>'+fullName+'</span></div>');
            $('div#nca'+idCounter).append('<div class=address>'+address+'</div></div></li>');
