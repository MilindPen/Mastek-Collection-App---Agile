...
var validateNewUser = {
	            ...
	            success:function (data) {
	            	if(data.success===false){
                                ...
                                if(data.errorCode == "ERR-204"){
                                    ...
                                    common.alertMessage(data.errorMessage, callbackReturn, messageboxUserRegPage.messageTitleError);
                                }
    ...
    sendNewRegistrationToDatabase: function(userid, title, fname, mname, lname, mac, encryptedPass, isActive){
    	var encryptedpwd = encryptedPass.toString();
            ...
            $.ajax({
	            ...
	            data: JSON.stringify(newUserRegistrationJSONReq),
                    success:function (data){
                    	if(data.success===false){
    	            		//var message = data.errorCode+": "+data.errorMessage;
    	            		common.alertMessage(data.errorMessage,callbackReturn, callbackReturn, messageboxCallService.messageTitleError);
...
var service = {
	    	...
	    	$.ajax({
	            ...
	            success:function (data) {
	            	if(data.success===false){
	            		...
	            		//var message = data.errorCode+": "+data.errorMessage;
	            		common.alertMessage(data.errorMessage,callbackReturn,messageboxCallService.messageTitleError);
		...
		checkUserIsActive: function(){
			//alert("user verification service called");
			isSyncingForLockedUser = false;
            ...
            error: function(){
				...
				common.alertMessage(messageboxCallService.errorConnectionCheckActiveUserServiceMsg,callbackReturn,messageboxCallService.messageTitleError);
            }
		...
		getCustomerInformation: function(){
	    	...
	    	$.ajax({
                    type: 'POST',
	            //url: CUSTOMER_SYNC_URL + "/getScheduledCustomers",
		...
		getTransactionHistory: function(agreementIdsList){
	    	...
	    	$.ajax({
	            ...
	            success:function (data) {
	            	if(data.success===false){
	            		//var message = data.errorCode+": "+data.errorMessage;
	            		common.alertMessage(data.errorMessage,function(){common.redirect("dashboard.html")}(),messageboxCallService.messageTitleError);
...
var syncCustomerUpdateService = {
		...
		postCustomerUpdateData: function(customerUpdateList){
	    	...
	    	$.ajax({
                    type: 'POST',
	            //url: CUSTOMER_SYNC_URL + "/updateCustomerDetails", 
...
var transactionDataRestoreService = {

		/***********************************service call to post transaction for sync********************************/
		restoreTransactionData: function(){
			...
			console.log(JSON.stringify(InputJSON));
			console.log("Calling Transaction restore service");
			console.log(TRANSACTION_SYNC_URL + "/transactions/gettransactions");
	    	$.ajax({
                    type: 'POST',
	            url: TRANSACTION_SYNC_URL + "/transactions/gettransactions",
				//url: "http://172.16.145.33:9092/transaction-rs/api/transactions/gettransactions",
				contentType: "application/json",
				//beforeSend: function (xhr) {
				//	xhr.setRequestHeader('Ocp-Apim-Subscription-Key', SUBSCRIPTION_KEY);
				//},
	            ...
	            success:function (data) {
					...
					if(data.success != true){
						...
						//pushOpMsg = "0";
						dataStorage.restoreDataIntoOfflineDB(data);
		            	
	            	}
	            },
	            error: function(x, t, m) {
					
					common.alertMessage(messageboxCallService.errorConnectionDataRestoreServiceMsg,callbackReturn, messageboxCallService.messageTitleError);
	                if(t==="timeout") {
	                	common.alertMessage(messageboxCommonMessages.timeout,callbackReturn,messageboxCommonMessages.messageTitleError);
	                } else {
	                    //alert(t);
...
var delegationServices = {

		getUserSelectionData: function(startDate,endDate,selectedBranchId,callback){
		...
		var JSONReq = {
	            ...
	            $.ajax({
	                type: 'POST',
		            url: BALANCING_SYNC_URL + "/balancing/users",
		            contentType: "application/json",
					//beforeSend: function (xhr) {
					//	xhr.setRequestHeader('Ocp-Apim-Subscription-Key', SUBSCRIPTION_KEY);
					//},
		...
		getCustDelegationData: function(startDate,endDate,journeyId,callback){
		...
		var JSONReq = {
	            ...
	            $.ajax({
	                type: 'POST',
		            url: CUSTOMER_SYNC_URL + "/customers/delegationCustomers",
		            contentType: "application/json",
					//beforeSend: function (xhr) {
					//	xhr.setRequestHeader('Ocp-Apim-Subscription-Key', SUBSCRIPTION_KEY);
					//},
		            ...
		            data: JSON.stringify(JSONReq),
	                    success:function (data){
	                    	if(data.success===false){
	                            console.log("Customer for delegation service error");
								console.log(data.errorMessage);
								common.alertMessage(data.errorMessage,callbackReturn,messageboxCallService.messageTitleError);
	                        }
	                        else{
		...
		saveDelegatedCustomers: function(startDate,endDate,journeyId,delegatedToUserId,isSelectAll,customerJourneyIdList,callback){
		...
		var JSONReq = {
						...
						"delegatedCustomers": customerJourneyIdList
					 };
	                             
	            console.log(JSONReq);
	            console.log("Sending request to save delegated customer");
	        
	            $.ajax({
	                type: 'POST',
		            url: CUSTOMER_SYNC_URL + "/customers/savedelegatedcustomers",
		            contentType: "application/json",
					//beforeSend: function (xhr) {
					//	xhr.setRequestHeader('Ocp-Apim-Subscription-Key', SUBSCRIPTION_KEY);
					//},
		            ...
		            data: JSON.stringify(JSONReq),
	                    success:function (data){
	                    	if(data.success===false){
	                            console.log("Customer for delegation service error");
								console.log(data.errorMessage);
								if(data.errorCode=="ERR-250"){
									common.alertMessage(messageboxDelegationPage.ERR250,callbackReturn,messageboxDelegationPage.messageTitleSuccess);
								}
								else
								{
									common.alertMessage(data.errorMessage,callbackReturn,messageboxCallService.messageTitleError);
