...
var SCList = {		
    ...
    getDailyCustListFromOfflineDB: function(dbObj){
        ...
        var selectedVisitDateParam = [dataStorage.getData(SELECTED_VISIT_DATE),dataStorage.getData(JOURNEY_ID)];
    ...
    getWeeklyCustListFromOfflineDB: function(dbObj){
    	...
    	var selecSCWeeklyParam = [dataStorage.getData(JOURNEY_ID)];
        var fullName=[];
	 
        //execute select query
	    dbObj.transaction(function(tx) {	  
			tx.executeSql(dbQueries.selectSCWeeklyListStatement,selecSCWeeklyParam,function(tx, res) {
    ...
    renderSCDailyList: function(len,res){
		...
		$('ul.dailyList').append('<li class="cust ui-li-static ui-body-inherit" id=list'+idCounter+' onclick=SCList.callCustDetails(\"'+listItem.CustomerNumber+'\",'+idCounter+')>');
              ...
              //AT RISK & VULNERABLE Notification
                if(listItem.AtRisk || (listItem.AtRisk && listItem.Vulnerable !== "N")){
                    $('li#list'+idCounter).append('<div class=riskIndicator><img class=indicatorIcon src="css/themes/images/icons-png/atrisk-icon.png" alt=""></div>');
                }
                else if(listItem.Vulnerable !== "N"){
                    $('li#list'+idCounter).append('<div class=riskIndicator><img class=indicatorIcon src="css/themes/images/icons-png/vulnerable-icon.png" alt=""></div>');
                }
    ...
    renderSCWeeklyList: function(lenWeekly,res){
		...
		for(i=currBucket;i<datesBucket.length;i++){
			...
			if(datesBucket[i]==listItem.VisitDate){
				$('ul#weeklyListDay'+currBucket).append('<li class="cust ui-li-static ui-body-inherit" id=weeklyDay'+currBucket+'List'+bucketListCounter+' onclick=SCList.callCustDetailsWeekly(\"'+listItem.CustomerNumber+'\",'+currBucket+','+bucketListCounter+')>');
                ...
                //AT RISK & VULNERABLE Notification
                if(listItem.AtRisk || (listItem.AtRisk && listItem.Vulnerable !== "N")){
                    $('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class=riskIndicator><img class=indicatorIcon src="css/themes/images/icons-png/atrisk-icon.png" alt=""></div>');
                }
                else if(listItem.Vulnerable !== "N"){
                    $('li#weeklyDay'+currBucket+'List'+bucketListCounter).append('<div class=riskIndicator><img class=indicatorIcon src="css/themes/images/icons-png/vulnerable-icon.png" alt=""></div>');
                }
