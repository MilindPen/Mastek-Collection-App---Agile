...
var selectedCustLoanDetails = {
    ...
    renderSelectedCustDetails: function(res){
		...
		var agrimnt = listItem.AgreementID;
        var loanAmount;
		...
		var selctAgreementNo = listItem.AgreementNumber;
		...
		$('ul.cust').append('<li id="transactionHistory" class="ui-li-border ui-li-static ui-body-inherit ui-first-child ui-last-child"><a href="#" onclick=selectedCustLoanDetails.showTransactionHistory("'+previousAgreementNumber+'","'+weeklyTermAmt+'","'+agrimnt+'","'+selctAgreementNo+'")>Last 13 weeks transactions</a></li>');        
    },
	
	/***************************************Convert Amount to float and fixed 2 decimal************************************************/
    showTransactionHistory: function(previousAgreementNo,weeklyTrmAmt,agreemntId,folio){
	
        dataStorage.setData('PREVIOUS_AGREEMENT_NUM',previousAgreementNo);
		dataStorage.setData('WEEKLY_TERM_AMOUNT',weeklyTrmAmt);
        dataStorage.setData('AGREEMENT_ID',agreemntId);
		dataStorage.setData(SELECTED_AGREEMENT_NUM,folio);
